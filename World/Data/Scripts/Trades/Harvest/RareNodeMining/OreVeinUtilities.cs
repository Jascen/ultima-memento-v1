...
namespace Server.Engines.Harvest
{
    public class OreVeinUtilities
    {
        ...
        public static IEnumerable<OreVeinConfig> Build(Map map, RegionPredicate regionPredicate)
        {
            ...
            return pointsByRegion.Keys
                .OrderBy(x => x)
                .ThenByDescending(x => x == OreVeinEngine.UNKNOWN_REGION_NAME)
                .Select(region =>
                {
        ...
        public static void GetCandidate(Map map, int x, int y, RegionPredicate regionPredicate, GotCandidateCallback onCandidateFound, bool requireMiningNodes, Action<string> onFailureMessage = null)
        {
            ...
            var regionName = OreVeinUtilities.GetRegionName(reg, map, point);
        ...
        {
            return region.Name ?? Misc.Worlds.GetRegionName(map, point) ?? OreVeinEngine.UNKNOWN_REGION_NAME;
        }
