				...
				foreach ( Item item in TitemsInRange )
				{
					...
					else if ( item is HiddenChest )
					{
						...
						else if ( level > 4 )
						{
							...
							Effects.SendLocationParticles( EffectItem.Create( mBox.Location, mBox.Map, EffectItem.DefaultDuration ), 0x376A, 9, 32, 0, 0, 5024, 0 );
							Caster.PlaySound( Caster.Female ? 779 : 1050 );
							Caster.Say( "*ah ha!*" );
							foundAnyone = true;
						}
						else
						{
							...
							Effects.SendLocationParticles( EffectItem.Create( coins.Location, coins.Map, EffectItem.DefaultDuration ), 0x376A, 9, 32, 0, 0, 5024, 0 );
							Caster.PlaySound( Caster.Female ? 779 : 1050 );
							Caster.Say( "*ah ha!*" );
							foundAnyone = true;
						}
