	...
	public abstract class BaseVendor : BaseCreature, IVendor
	{
		...
		public override void AddCustomContextEntries( Mobile from, List<ContextMenuEntry> list )
		{
			if ( from.Alive && IsActiveVendor )
			{
				...
				if ( IsBlackMarket && this.BankBox.TotalItems > 0 && MyServerSettings.BlackMarket() )
			...
			if ( VendorRepairs( this ) )
				list.Add( new RepairEntry( from, this ) );

			if ( RelicFunctions.VendorDoesID( this ) )
				list.Add( new IdentifyEntry( from, this ) );

			if ( MagicRecharge( this ) )
        ...
        private class IDTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
			{
				if ( targeted is Item )
				{
					...
					if ( examine.CoinPrice > 0 && examine.NotIdentified && RelicFunctions.VendorCanID( examine.NotIDSource, m_Vendor ) )
					{
...
namespace Server.ContextMenus
{
	...
	public class BlackMarketEntry : ContextMenuEntry
	{
		...
		public BlackMarketEntry( Mobile from, BaseVendor vendor ) : base( 6094, 8 )
		{
			m_Vendor = vendor;
			m_From = from;
			Enabled = vendor.CheckVendorAccess( from );
